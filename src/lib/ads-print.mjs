...
/** @typedef {import('./ads-client.mjs').MetricRow} MetricRow */
...
/** @typedef {import('./ads-plan.mjs').DecideResult} DecideResult */
/** @typedef {import('./ads-plan.mjs').TermDecision} TermDecision */
/** @typedef {import('./ads-plan.mjs').HeldTerm} HeldTerm */
/** @typedef {import('./ads-plan.mjs').Promotion} Promotion */
/** @typedef {import('./ads-plan.mjs').ConvertingTerm} ConvertingTerm */
/** @typedef {import('./ads-plan.mjs').MiningArtifact} MiningArtifact */

/** @param {{flags: Flags, level: string, from: string, to: string, org: string, metrics: MetricRow[], money$: MonetisationSignal}} opts @returns {number|void} */
export function printReport({ flags, level, from, to, org, metrics, money$ }) {
	...
	info(`total ${c.bold(money(spend))} · ${c.bold(String(installs))} installs · blended CPI ${c.bold(installs ? money(spend / installs) : '—')} · CPT ${c.bold(taps ? money(spend / taps) : '—')}`);
	printMonetisationLine(money$);
	printReportNotes(level, metrics);
}

/** @param {MonetisationSignal} money$ @returns {void} */
...
/** @param {string} level @param {MetricRow[]} metrics @returns {void} */
...
/** @param {{campaigns: SnapshotCampaign[]}} account @param {{org: string, from: string, to: string, file: string}} opts @returns {void} */
...
/** @param {{decided: DecideResult, artifact: MiningArtifact, converting: ConvertingTerm[], gap: {staged: boolean, missing: string[]}, outFile: string, paid: {file: string, count: number}, locale: string, from: string, to: string, wantsApply: boolean, confirmed: boolean}} opts @returns {void} */
export function printMining({ decided, artifact, converting, gap, outFile, paid, locale, from, to, wantsApply, confirmed }) {
	...
	/** @template T @param {string} title @param {T[]} rows @param {{header: string, get: (row: T) => import('./util.mjs').Json}[]} cols @param {string} [empty] @returns {void} */
...
/** @param {{staged: boolean, missing: string[]}} gap @param {string} locale @returns {void} */
...
/** @param {{wantsApply: boolean, confirmed: boolean, artifact: MiningArtifact, decided: DecideResult, outFile: string}} opts @returns {void} */
