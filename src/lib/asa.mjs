// Apple Search Ads decision core: bids, the kill rule, and reconciliation.
//
...
const clamp100 = (n) => Math.min(100, Math.max(0, num(n)));
...
function reconcileOne({ level, path, name, desired, live, synced, force, adopt }) {
	...
	if (firstTime) {
		// Never synced, so every live value is a human's. Adopting the id is safe;
		// overwriting the values they chose is the thing that must be asked for.
...
/**
 * The whole plan against the whole account, as a list of transitions a human can
 * read before any of them happen. Pure, so `--dry-run` and a real run print the
 * same list by construction rather than by two code paths agreeing.
 *
...
export function reconcile({ planned = [], live = [], force = false, adopt = false, prune = false } = {}) {
	...
	for (const cp of planned) {
		...
		for (const g of plannedGroups) {
			...
			const liveKeywords = gHit.row?.keywords ?? [];
			...
			for (const k of g.keywords ?? []) {
				...
				if (kHit.row) claimedKeywords.add(kHit.row.id);
				actions.push(
					reconcileOne({
						level: 'keyword',
						path: `${cp.name} / ${g.name} / ${want.text} ${want.matchType}`,
			...
			for (const k of liveKeywords) {
				if (claimedKeywords.has(k.id) || String(k.status ?? '').toUpperCase() === 'PAUSED') continue;
				actions.push({
					level: 'keyword',
					path: `${cp.name} / ${g.name} / ${k.text} ${k.matchType}`,
					name: `${k.text} ${k.matchType}`,
					id: k.id,
					op: prune ? 'pause' : 'unplanned',
					changes: [{ field: 'status', from: k.status ?? null, to: 'PAUSED' }],
					detail: 'live but absent from the plan',
				});
			}
		}
		for (const g of liveGroups) {
			if (claimedGroups.has(g.id) || String(g.status ?? '').toUpperCase() === 'PAUSED') continue;
			actions.push({
				level: 'adGroup',
				path: `${cp.name} / ${g.name}`,
				name: g.name,
				id: g.id,
				op: prune ? 'pause' : 'unplanned',
				changes: [{ field: 'status', from: g.status ?? null, to: 'PAUSED' }],
				detail: 'live but absent from the plan',
			});
		}

		const liveNegatives = hit.row?.negativeKeywords ?? [];
		for (const n of cp.negativeKeywords ?? [])
			if (!liveNegatives.some((h) => keywordKey(h) === keywordKey(n)))
				actions.push({
					level: 'negative',
					path: `${cp.name} / −${n.text} ${n.matchType}`,
